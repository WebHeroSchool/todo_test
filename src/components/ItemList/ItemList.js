        ...
        <div>
          {filter.length !== 0
            ? <ul className={styles.wrap}>
              {filter.map(item => (<li key={item.value}>
                <div className={styles.item__wrap}>
                  <FormControlLabel className={styles.checkbox}
                    control={
                        <Checkbox
                            checked={item.isDone}
                            icon={<RadioButtonUncheckedIcon className={styles.checkbox__icon}/>}
                            checkedIcon={<CheckCircleIcon className={styles.checkbox__icon}/>}
                            value="checkedB"
                            color="primary"
                    ...
                    <Tooltip title="Удалить" onClick={() => onClickDelete(item.id)}>
                      <IconButton aria-label="delete">
                        <CancelIcon fontSize="small" className={styles.delete__icon}/>
                      </IconButton>
                    </Tooltip>
                </div>
              </li>
